...
let shrinkingGuestList: string[] = ["Baba", "Kamal", "Shah", "Hasil", "Saim", "Jamal", "Abrar"];

console.log(
  "Unfortunately! our new dinner table won't arrive in the time, so we can only invite two people"
);

// removing guest from the list
while (shrinkingGuestList.length > 2) {
  let removeGuest: string | undefined = shrinkingGuestList.pop();
...
shrinkingGuestList.forEach((guest) =>
